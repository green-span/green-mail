...
#        print 'C: ' + repr(line)
    ...
    def _regularDispatch(self, line):
        """ had to change this for IDLE command

        IDLE server response is '+ idling'. A valid response tag is not
        returned until client sends DONE command.

        Added this hack since we expect a tag in response to client command. 
    ...
    def makeTag(self, cmd):
        """ had to change this for IDLE command 
        see notes in _regularDispatch """
        if cmd.command == 'IDLE':
            tag = 'I001'
        else:
            tag = '%0.4X' % self.tagID
